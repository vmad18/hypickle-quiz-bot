...
import mod.quizbot.me.QuizFactory;
import mod.quizbot.me.Threads.MessageRunnable;
import mod.quizbot.me.Threads.PlayerMessageThread;
...
public class QuizEvents {
    ...
    public String getName(String text, int stage) {
        ...
        try {
            ...
            if (splits[0].equals("From")) {
                ...
                if (bot.getPlayer(name).getPhase() == stage || stage == 0) return name;
            }
        } catch (Exception e) {
            return "";
        }
    ...
    public void runMessages(List<String> q, String name) {
        for (int i = 0; i < q.size(); i++)
            bot.submitThread(new MessageRunnable(4 + 3 * i, "/msg " + name + q.get(i), bot.getMc()));
    }
    ...
    public void onFriend(ClientChatReceivedEvent e) {
        ...
        if (friendMatcher.matches()) {
            ...
            bot.store.put(name, new PlayerData(name));
        }
    }
    ...
    public void setSubject(ClientChatReceivedEvent e) {
        ...
        bot.getPlayer(name).setSubject(a);
    ...
    public void setQuestionSize(ClientChatReceivedEvent e) throws InvocationTargetException, IllegalAccessException {
        ...
        bot.getPlayer(name).setCount(num);

        bot.submitThread(new MessageRunnable(2, "/msg " + name + " Keep your answer in the form of 'Answer: <ans>", bot.getMc()));

        //Fire first question
        QuizFactory q = new QuizFactory(bot.getPlayer(name).getSubject());
        runMessages(q.getQuestions(), name);

        //Store first answer
        bot.getPlayer(name).getKey().addElm(q.getAns());
    ...
    public void ansList(ClientChatReceivedEvent e) throws InvocationTargetException, IllegalAccessException {
        ...
        bot.getPlayer(name).getAnswers().addElm(Double.parseDouble(a));

        if (bot.getPlayer(name).getAnswers().getArr().size() == bot.getPlayer(name).getCount()) {
    ...
    public void calculator(ClientChatReceivedEvent e) {
        ...
        if (msg.contains(bot.getMc().thePlayer.getName())) {
            ...
            if (blackList.contains(bot.getPlayer(name).getSubject())) {
    ...
    private double validate(String name) {
        List<Double> ans = bot.getPlayer(name).getAnswers().getArr(), key = bot.getPlayer(name).getKey().getArr();
        double correct = 0;
    ...
    //TODO Add a Rating System (kinda dumb)
